...
float inline genann_act_sigmoid(const genann *ann unused, float a) {
  if (a < -45.0) return 0;
  if (a > 45.0) return 1;
  return 1.0 / (1 + exp(-a));
}
...
float inline genann_act_sigmoid_cached(const genann *ann unused, float a) {
...
float inline genann_act_linear(const struct genann *ann unused, float a) {
  return a;
}

float inline genann_act_threshold(const struct genann *ann unused, float a) {
  return a > 0;
}
